/*
  Sync Next.js app routes (e.g., app/.../page.tsx) into Sanity `page` documents so editors can pick from an auto-generated list.
...
// Provide __dirname in ESM/TS environments
...
type PageInfo = { route: string; title: string; slug: string; id: string };
...
function toSlugFromRoute(route: string): string {
  // e.g., "/about/staff" -> "about-staff"; "/" -> "home"
  if (route === '/') return 'home';
  return route.replace(/^\//, '').replace(/\//g, '-').replace(/^-+|-+$/g, '') || 'home';
}

function idFromRoute(route: string): string {
  // Stable id, only [a-zA-Z0-9_-]
  const safe = route.replace(/[^a-zA-Z0-9/_-]/g, '').replace(/\//g, '__') || 'root';
  return `page__${safe}`;
}

function collectRoutes(dir: string, baseSegments: string[] = []): PageInfo[] {
  const entries = fs.readdirSync(dir, {withFileTypes: true});
  let pages: PageInfo[] = [];

  // If this directory contains page.tsx, record a route
  ...
  for (const entry of entries) {
    if (entry.isDirectory()) {
      ...
      if (isDynamic(name)) continue; // skip dynamic
...
async function run(): Promise<void> {
  ...
  const pages = collectRoutes(appDir)
    // Exclude api routes and special folders
